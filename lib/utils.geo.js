 ...
 */	
function getPoint(query,idx,type,callback){
	if(geoservices && geoservices.length && idx<geoservices.length){
		var url = geoservices[idx][type+"url"];
		if(typeof query=="object"){
			query = query.join(",");
		}
		urlutils.get(url+encodeURIComponent(query.toUnicode()),function(js){
			if(js){
				...
				if(n){
					...
					if(n && n[geoservices[idx].lat] && n[geoservices[idx].lon]){
						r["lat"] = n[geoservices[idx].lat].tof();
						r["lon"] = n[geoservices[idx].lon].tof();
 ...
 * @return {String}
 */	
exports.rgeocode = function(latitude, longitude, callback){
	...
	getPoint([latitude,longitude],0,"r",callback);
}
