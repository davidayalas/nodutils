	...
	if(cache[key]){
		...
		fileutils.getModTime(self.getPath()+"/"+key,function(moddate){
			if(moddate==null){ //doesn't exist
				...
				fileutils.read(self.getPath()+"/"+key,function(err,data){
					if(!err && data!=null){ //has content, seconds in first line
...
exports.remove = function(key, callback){
	...
	delete cache[key];
	fileutils.exists(self.getPath()+"/"+key, function(exists){
		if(exists){
			fileutils.remove(self.getPath()+"/"+key, function(err){
				console.log(err)
				if(typeof(callback)=="function"){
					callback();
				}
...
exports.show = function(){
	return {"cache":cache, "size" : cacheSize}
}